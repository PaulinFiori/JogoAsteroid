...
function love.load()
    sound = love.audio.newSource("Audios/SpaceShotButtonClick.mp3", "static")
    optionsback = love.graphics.newImage("sprites/OptionsBack.png")
    startambient = love.audio.newSource("Audios/Flight.mp3", "static")
...
function love.draw()
    ...
    if hideStartMenu == false and StartGame == false then love.graphics.draw(back, 0, 0) end

    if hideStartMenu == true and StartGame == false then 
...
function love.update(dt)
    ...
    if hideStartMenu == false and StartGame == false then
    ...
    if hideStartMenu == true and StartGame == false then
        ...
        suit.Label("Reiniciar:                  R", 470,520, 195,30)

        if suit.Button("Return to menu", 90,555, 200,35).hit then
            sound:play()
            hideStartMenu = false;
        end
    end
